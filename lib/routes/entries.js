...
exports.register = function(server, options, next) {
  ...
  server.route({  
    ...
    handler: async (request, reply) => {
      const { start_date, end_date } = request.query.params;
  ...
  server.route({  
    ...
    handler: async (request, reply) => {
      const week_string = request.params.week_string;
      ...
      try {
        ...
        const res = {
          ...
          daily_entries: _.filter(entries, { 'is_weekly': false }),
          weekly_entries: _.filter(entries, 'is_weekly')
        };
        return reply(res);
      } catch (e) {
        console.error(e);
    ...
    config: {
      description: 'Get entries for the specified week string in formatted return object.'
    }
  });
